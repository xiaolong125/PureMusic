...
class LyricView : View {
    ...
    private var mHintColor = Color.parseColor("#666666") // 提示语颜色
    private var mDefaultColor = Color.parseColor("#000000") // 默认字体颜色
    private val mIndicatorColor = Color.parseColor("#EFEFEF") // 指示器颜色
    private var mHighLightColor = Color.parseColor("#4FC5C7") // 当前播放位置的颜色
    private val mCurrentShowColor = Color.parseColor("#AAAAAA") // 当前拖动位置的颜色
    private var mLineCount:Int = 0 // 行数 = 0
    private var mLineHeight:Float = 0f// 行高 = 0f
    ...
    private val mMinStartUpSpeed = 1600 // 最低滑行启动速度
    private var mUserTouch = false // 判断当前用户是否触摸
    private var mIndicatorShow = false // 判断当前滑动指示器是否显示
    private var mIsMoved = false //判断用户触摸时是否发生move事件
    private var mPlayerClick = false //判断当前用户是否点击指示器

    /** */
    private var mBtnWidth = 0 // Btn 按钮的宽度
    private val mDefaultMargin = 12
    private var maximumFlingVelocity :Int = 0 // 最大纵向滑动速度 = 0
    ...
    override fun onDraw(canvas: Canvas) {
        if (mLyricInfo != null && mLyricInfo!!.song_lines != null && mLyricInfo!!.song_lines!!.size > 0) {
            ...
            canvas.drawText(
                mDefaultHint,
                measuredWidth * 0.5f,
                (measuredHeight + mLineHeight - 6) * 0.5f,
                mTextPaint!!
            )
        }
        /**
    ...
    private fun drawPlayer(canvas: Canvas) {
        ...
        val value = Math.sqrt(
            Math.pow(
                radio.toDouble(),
                2.0
            ) - Math.pow(radio * 0.5f.toDouble(), 2.0)
        ).toFloat()
        ...
        canvas.drawCircle(
            mBtnBound!!.centerX().toFloat(),
            mBtnBound!!.centerY().toFloat(),
            mBtnBound!!.width() * 0.48f,
            mBtnPaint!!
        ) // 绘制圆环
    }
    ...
    private fun drawIndicator(canvas: Canvas) {
        ...
        path.moveTo(
            if (mPlayable) mBtnBound!!.right + 24f else 24.toFloat(),
            measuredHeight * 0.5f
        )
        path.lineTo(
    ...
    private var mLastScrollY = 0f // 记录手指按下时的坐标和当前的滑动偏移量 = 0f
    ...
    private fun actionMove(event: MotionEvent) {
        if (scrollable()) {
            ...
            val scrollY = mLastScrollY + mDownY - event.y // 102  -2  58  42
            val value01 =
                scrollY - mLineCount * mLineHeight * 0.5f // 52  -52  8  -8
            val value02 =
                Math.abs(value01) - mLineCount * mLineHeight * 0.5f // 2  2  -42  -42
            mScrollY =
    ...
    private fun doFlingAnimator(velocity: Float) {
        ...
        val distance = velocity / Math.abs(velocity) * Math.min(
            Math.abs(velocity) * 0.050f, 640f
        ) // 计算就已当前的滑动速度理论上的滑行距离是多少
        val to = Math.min(
            Math.max(0f, mScrollY - distance),
            (mLineCount - 1) * mLineHeight
        ) // 综合考虑边界问题后得出的实际滑行距离
        mFlingAnimator = ValueAnimator.ofFloat(mScrollY, to)
        mFlingAnimator?.addUpdateListener(AnimatorUpdateListener { animation ->
    ...
    var postman: Handler = object : Handler() {
     ...
     */
    private fun setupLyricResource(
    ...
    private fun analyzeLyric(lyricInfo: LyricInfo, line: String?) {
        ...
        if (line != null && index == 9 && line.trim { it <= ' ' }.length > 10) {
            ...
            val content = line.substring(lastIndexOfRightBracket + 1, line.length)
            val times =
    ...
    internal inner class LyricInfo {
        var song_lines: MutableList<LineInfo>? =
            null
        var song_artist // 歌手
                : String? = null
        var song_title // 标题
    ...
    internal inner class sort :
        ...
        override fun compare(
            ...
            return if (lrc.start < lrc2.start) {
                -1
